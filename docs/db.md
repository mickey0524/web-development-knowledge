...
* 在什么情况下会发生针对该列创建了索引但是在查询的时候并没有使用呢
    ...
    1. 使用不等于查询
    2. 列参与了数学运算或者函数
    3. 在字符串 like 时左边是通配符，类似于 '%aaa'
    4. 当 MySQL 分析全表扫描比使用索引快的时候不使用索引
...
* [MySQL面试高频一百问(工程师方向)](https://juejin.im/post/5d351303f265da1bd30596f9?utm_source=gold_browser_extension)

* [关于(缓存穿透、缓存击穿、缓存雪崩、热点数据失效)问题的解决方案](https://juejin.im/post/5c9a67ac6fb9a070cb24bf34)

* Redis 和 MySQL 缓存一致性的问题如何解决
